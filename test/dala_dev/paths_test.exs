defmodule DalaDev.PathsTest do
  ...
  setup do
    ...
    tmp =
      Path.join(System.tmp_dir!(), "dala_paths_test_#{System.unique_integer([:positive])}")
  ...
  describe "default_runtime_dir/0" do
    test "is under ~/.dala/runtime/ios-sim" do
  ...
  describe "build_sh_aware?/1" do
    ...
    test "true when build.sh contains DALA_SIM_RUNTIME_DIR", %{project: project} do
      File.write!(
  ...
  describe "sim_runtime_dir/1" do
    test "DALA_SIM_RUNTIME_DIR env wins over everything", %{project: project} do
      System.put_env("DALA_SIM_RUNTIME_DIR", "/somewhere/else")

      try do
        # Even with a build.sh-aware project, the env var wins.
        File.write!(Path.join([project, "ios", "build.sh"]), "DALA_SIM_RUNTIME_DIR\n")
