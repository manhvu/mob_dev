defmodule DalaDev.ConnectorTest do
  ...
  describe "handle_dist_start/2" do
    test "raises Mix.Error with epmd hint on generic failure" do
      assert_raise Mix.Error, ~r/epmd -daemon/, fn ->
        Connector.handle_dist_start({:error, :econnrefused}, :dala_secret)
      end
    end
    ...
    test "error message points to mix dala.doctor" do
      assert_raise Mix.Error, ~r/mix dala\.doctor/, fn ->
        Connector.handle_dist_start({:error, :something_else}, :dala_secret)
      end
    end

    test "error message includes the retry instruction" do
      assert_raise Mix.Error, ~r/mix dala\.connect/, fn ->
        Connector.handle_dist_start({:error, :enoent}, :dala_secret)
      end
    end
