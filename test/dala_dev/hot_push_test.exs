defmodule DalaDev.HotPushTest do
  ...
  describe "push_changed/2" do
    ...
    test "detects beam files not in snapshot (empty snapshot)" do
      ...
      # push_changed only counts runtime deps — not dev-only deps like dala_dev itself.
  ...
  describe "push_all/1" do
    ...
    test "push count is less than total beam files in _build" do
      # push_all only pushes runtime deps — dev-only deps (dala_dev itself, Bandit,
