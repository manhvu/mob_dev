defmodule DalaDev.NativeBuildTest do
  ...
  describe "read_sdk_dir/1" do
    setup do
      tmp =
        Path.join(
          System.tmp_dir!(),
          "dala_native_build_test_#{System.unique_integer([:positive])}"
        )
  ...
  describe "narrow_platforms_for_device/2 and /3" do
    ...
    test "discovery hit by display_id (8-char prefix) still drops Android" do
      # `mix dala.devices` prints a short display id (first 8 chars of
    ...
    test "/2 form delegates to /3 with the real iOS discovery (smoke check)" do
      ...
      # use (mix dala.deploy, native_build.build_all).
  ...
  # format-only fallback without hitting `DalaDev.Discovery.IOS.list_devices/0`.
