defmodule DalaDev.Bench.DeviceObserverTest do
  use ExUnit.Case, async: true

  alias DalaDev.Bench.{DeviceObserver, Probe}
  ...
  describe "consume_messages/1" do
    ...
    test "drains messages from the mailbox", %{obs: obs} do
      send(self(), {:dala_device, :screen_off})
      send(self(), {:dala_device, :did_enter_background})
    ...
    test "events from consume_messages preserved newest-first", %{obs: obs} do
      send(self(), {:dala_device, :screen_off})
      send(self(), {:dala_device, :screen_on})
    ...
    test "messages with payload are accepted", %{obs: obs} do
      send(self(), {:dala_device, :thermal_state_changed, :serious})
    ...
    test "non-dala_device messages are not consumed (left in mailbox)", %{obs: obs} do
      ...
      # The non-dala_device messages should still be there.
