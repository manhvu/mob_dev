defmodule Mix.Tasks.Dala.InstallTest do
  ...
  describe "replace_prop/3" do
    test "replaces a matching key=value line" do
      content = "dala.otp_release=/path/to/placeholder\ndala.dala_dir=/path/to/dala\n"
      result = Install.replace_prop(content, "dala.otp_release", "/new/path")
      assert result =~ "dala.otp_release=/new/path\n"
      refute result =~ "placeholder"
    end

    test "leaves other keys untouched" do
      content = "dala.otp_release=/old\ndala.dala_dir=/my/dala\n"
      result = Install.replace_prop(content, "dala.otp_release", "/new")
      assert result =~ "dala.dala_dir=/my/dala"
    end

    test "is a no-op when value is nil" do
      content = "dala.otp_release=/old\n"
      assert Install.replace_prop(content, "dala.otp_release", nil) == content
    end

    test "replaces dala.otp_release_arm32 independently" do
      content = "dala.otp_release=/arm64\ndala.otp_release_arm32=/placeholder\n"
      result = Install.replace_prop(content, "dala.otp_release_arm32", "/arm32/real")
      assert result =~ "dala.otp_release_arm32=/arm32/real"
      assert result =~ "dala.otp_release=/arm64"
    end
  end
  ...
  describe "write_local_properties/2" do
    setup do
      dir = Path.join(System.tmp_dir!(), "dala_install_#{System.unique_integer([:positive])}")
    ...
    defp write_placeholder_props(dir) do
      ...
      dala.otp_release=/path/to/otp-android
      dala.otp_release_arm32=/path/to/otp-android-arm32
      dala.dala_dir=/path/to/dala
      """)
    end
    ...
    test "writes arm64 OTP path when placeholder present", %{dir: dir} do
      ...
      Install.write_local_properties(dir, dala_dir: dir)
      content = File.read!(props_path(dir))
      assert content =~ "dala.otp_release=#{OtpDownloader.android_otp_dir("arm64-v8a")}"
    end
    ...
    test "writes arm32 OTP path when placeholder present", %{dir: dir} do
      ...
      Install.write_local_properties(dir, dala_dir: dir)
      content = File.read!(props_path(dir))
      assert content =~ "dala.otp_release_arm32=#{OtpDownloader.android_otp_dir("armeabi-v7a")}"
    end
    ...
    test "does nothing when local.properties is fully populated", %{dir: dir} do
      # Both Dala paths and sdk.dir already set — write_local_properties should
      # leave it byte-identical.
      original =
        "sdk.dir=/already/set/sdk\ndala.otp_release=/already/set\ndala.dala_dir=/already/set\n"
  ...
  describe "has_active_sdk_dir?/1" do
    ...
    test "false when the line is missing entirely" do
      refute Install.has_active_sdk_dir?("dala.dala_dir=/foo\n")
    end
  end
  ...
  describe "ensure_sdk_dir/2" do
    test "no-op when sdk_path is nil" do
      assert Install.ensure_sdk_dir("dala.dala_dir=/foo\n", nil) == "dala.dala_dir=/foo\n"
    end
  ...
  describe "detect_android_sdk/0" do
    test "returns ANDROID_HOME when set and the directory exists" do
      tmp =
        Path.join(System.tmp_dir!(), "dala_sdk_test_#{System.unique_integer([:positive])}")
