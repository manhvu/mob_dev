defmodule Mix.Tasks.Dala.CacheTest do
  # async: false because tests mutate process-global env vars (DALA_CACHE_DIR
  # and DALA_SIM_RUNTIME_DIR) — running them in parallel with PathsTest would
  # race.
  ...
  describe "our_cache/0" do
    test "honors DALA_CACHE_DIR" do
      System.put_env("DALA_CACHE_DIR", "/tmp/explicitly_set_cache")

      try do
        assert %{path: "/tmp/explicitly_set_cache", kind: :ours} = Cache.our_cache()
      after
        System.delete_env("DALA_CACHE_DIR")
      end
    end

    test "falls back to ~/.dala/cache when DALA_CACHE_DIR is unset" do
      System.delete_env("DALA_CACHE_DIR")
      home = System.user_home!()
      assert %{path: path, kind: :ours} = Cache.our_cache()
      assert path == Path.join([home, ".dala", "cache"])
    end
  end
  ...
  describe "sim_runtime_targets/0" do
    test "always lists the new default and the legacy /tmp path" do
      ...
      assert DalaDev.Paths.default_runtime_dir() in paths
      assert DalaDev.Paths.legacy_tmp_path() in paths
    end

    test "adds DALA_SIM_RUNTIME_DIR override when it differs from defaults" do
      System.put_env("DALA_SIM_RUNTIME_DIR", "/somewhere/exotic")
      ...
      after
        System.delete_env("DALA_SIM_RUNTIME_DIR")
      end
    end

    test "deduplicates when override matches the default" do
      default = DalaDev.Paths.default_runtime_dir()
      System.put_env("DALA_SIM_RUNTIME_DIR", default)
  ...
  describe "path_status/1" do
    setup do
      tmp =
        Path.join(System.tmp_dir!(), "dala_cache_test_#{System.unique_integer([:positive])}")
