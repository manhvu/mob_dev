defmodule Mix.Tasks.Dala.BatteryBenchAndroid do
  use Mix.Task
  ...
  def run(args) do
      ...
      case opts[:device] || auto_detect_device() do
        nil ->
          ...
          No Android device found. Options:
            mix dala.battery_bench_android --device 192.168.1.42:5555
    ...
    unless adb_ok?(device) do
      ...
      You can do it manually before re-running:
        ...
        mix dala.battery_bench_android --no-build --device <PHONE-WIFI-IP>:5555
      """)
    end
    ...
    # before dala_start_beam runs. If we set up tunnels after launch, the
    ...
    suffix = DalaDev.Discovery.Android.device_node_suffix(device)
    ...
    reconnector = Reconnector.new(active_node || :unset@unset, :dala_secret)
  ...
  def resolve_build_flags(opts) do
    cond do
      ...
      opts[:flags] ->
        ...
        header =
          "/* generated by mix dala.battery_bench_android -- do not edit */\n" <>
  ...
  defp install_apk(device, apk, pkg) do
    ...
    # `mix dala.deploy --native` during initial provisioning. A previous
    # version of this code did `adb uninstall && adb install`, which nuked
    # /data/data/ and left the device with no ERTS, so dala_start_beam would
    ...
    # runtime via the next `mix dala.deploy --native`, but the user has been
    # warned.
  ...
  defp push_beams_runas(device, pkg, beams_dir, beam_dirs) do
    stage_local = Path.join(System.tmp_dir!(), "dala_bench_beams.tar")
    stage_device = "/data/local/tmp/dala_bench_beams.tar"

    tmp = Path.join(System.tmp_dir!(), "dala_bench_stage")
    File.rm_rf!(tmp)
    File.mkdir_p!(tmp)
  ...
  defp beam_reachable?(node) do
    Node.set_cookie(node, :dala_secret)
  ...
  defp crash_diagnosis_no_process(device, pkg) do
    ...
    keep it alive. Common causes:
      ...
      - Missing ERTS helper libs in the APK (check lib/<abi>/ contains
        ...
        lib/arm64).
      - Bad BEAM flags in dala.exs (try `mix dala.deploy --beam-flags ""`)
      - App crashed for an unrelated reason — check logcat:

          adb -s #{device} logcat -d | grep -iE "DalaBeam|DalaNIF|FATAL|tombstone"
  ...
  defp crash_diagnosis_no_beam(device, pkg, pid) do
    ...
    causes:
      ...
      - BEAM crashed in dala_start_beam — check logcat for SIGABRT in
        beam-main:

          adb -s #{device} logcat -d | grep -iE "DalaBeam|FATAL|SIGABRT|beam-main"
      ...
      - OTP runtime never deployed to this device. The app is installed
        ...
        the device wasn't connected during a previous `mix dala.deploy
        --native`. Provision it now:

          mix dala.deploy --native --device #{device}

      - BEAMs stale on device. If OTP is present, push fresh BEAMs:

          mix dala.deploy --android --device #{device}
  ...
  defp stale_epmd_recovery_hint(device, pkg) do
    ...
    #{collision_block}
       Other recovery options:
         ...
         # Force EPMD to forget every node (kills any other Dala IEx sessions):
        ...
        if String.contains?(out, "connected") or String.contains?(out, "already connected") do
          ...
          if adb_ok?(new_device) do
            ...
            Check WiFi network and re-run with the WiFi-ADB serial:
              mix dala.battery_bench_android --no-build --device #{new_device}
            """)
          end
        else
          Mix.raise("""
