defmodule Mix.Tasks.Mob.Screen do
  use Mix.Task
  ...
  ## Examples
      ...
      mix mob.screen --capture

      # Save screenshot to file
      mix mob.screen --capture screenshot.png

      # Record 30 seconds of video
      mix mob.screen --record --duration 30

      # Start live preview in browser
      mix mob.screen --preview

      # Specify device by node
      mix mob.screen --node mob_qa@192.168.1.5 --capture
  ...
  alias MobDev.{ScreenCapture, Device, Discovery}
  ...
  defp get_device_ref(opts) do
    case Keyword.get(opts, :node) do
      ...
      node_str ->
        ...
        case node_str do
          "mob_qa@" <> _ = node_str -> String.to_atom(node_str)
          _ -> node_str
        end
    end
  end
  ...
  defp show_usage do
    ...
    Examples:
      mix mob.screen --capture
      mix mob.screen --record --duration 60
      mix mob.screen --preview --port 8080
    """)
  end
end
