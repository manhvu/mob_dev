defmodule Mix.Tasks.Mob.Provision do
  ...
  defp generate_xcodeproj(bundle_id, team_id, profile_specifier) do
    ...
    needs_write =
      case File.read(proj_file) do
        ...
        # newer mob_dev versions, hand-edits — gets rewritten. Cheap to
        # do (one local file write) and catches the common "old project
        # generated by an older mob_dev" trap on upgrade.
        _ -> true
      end
  ...
  #
  # Reuses MobDev.Release.parse_mobileprovision/1 which already filters
    ...
    cond do
      snippet = match_no_store_profile(output) ->
        {"Distribution profile can't be auto-created for an unregistered App ID", snippet,
         ...
         Register the App ID once (1 minute), then re-run mix mob.provision:
         ...
         Then: mix mob.provision --distribution
         """}
