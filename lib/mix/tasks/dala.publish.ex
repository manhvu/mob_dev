defmodule Mix.Tasks.Dala.Publish do
  use Mix.Task
  ...
  def run(argv) do
    ...
    case :os.type() do
      {:unix, :darwin} ->
        :ok
        Mix.raise("mix dala.publish is only supported on macOS (xcrun altool is required).")
    end
  ...
  defp resolve_ipa_path([]) do
    ...
    case Path.wildcard(Path.join(output_dir, "*.ipa")) do
      [] ->
        ...
        Run `mix dala.release` first, or pass an explicit path:

            mix dala.publish path/to/App.ipa
        """)
      ...
      many ->
        ...
        #{Enum.map_join(many, "\n", &"    #{&1}")}
            ...
            mix dala.publish #{List.first(many)}
        """)
    end
  end

  defp resolve_ipa_path(_) do
    Mix.raise("Usage: mix dala.publish [path/to/App.ipa]")
  end
  ...
  defp load_asc_config! do
    ...
    unless File.exists?(config_file) do
      Mix.raise("dala.exs not found in #{File.cwd!()} — run from the project root.")
    end
    ...
    unless is_list(asc) do
      Mix.raise("""
      Missing :app_store_connect in dala.exs. Add:
    ...
    Enum.each([:key_id, :issuer_id, :key_path], fn key ->
      unless is_binary(asc[key]) do
        Mix.raise("app_store_connect[:#{key}] missing or not a string in dala.exs")
      end
    end)
